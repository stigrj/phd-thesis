...
the concept of multiresolution analysis (MRA)\footnote{Mallat\cite{Mallat:1989} uses the term 
...
\end{equation}
where $H^{(m)}$ are the so-called filter matrices that describe the 
...
the basis functions $\scalingvec$ and $\waveletvec$, and the filter matrices $H^{(m)}$ 
and $G^{(m)}$ used in the two-scale relations Eq.~(\ref{eq:twoscalescaling}) and 
...
the space \scalingspace{0} for $0\leq m \leq k$, and since $\waveletspace{n} 
\perp \scalingspace{0}$ for all $n>=0$, the first $k+1$ moments of 
$\wavelet^n_j$ must vanish.
...
of $d$ mono-dimensional filters. A detailed discussion on multi-dimensional MRAs
and wavelet transforms can be found in Tymczak \etal\cite{Tymczak:2002}.
...
\end{equation}
Algorithms for how to achieve this accuracy is presented in 
Chap.~\ref{chap:implementation}.
...
described fully in the implementation part in Chap.~\ref{chap:implementation}. 
...
in detail in Chap.~\ref{chap:implementation}. Beylkin \etal~\cite{Beylkin:1991} 
...
\begin{align}
    \big[\tau_{lm}^{n}\big]_{ij} &= \int_{2^{-n}l}^{2^{-n}(l+1)} 
